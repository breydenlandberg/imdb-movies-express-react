...
describe('API', () => {
  ...
  test('GET request to movies/attributes (movie_attributes) for a specific movie using movie_id returns content type JSON', async () => {
  ...
  test('GET request to joined tables using name_id and advanced query returns content type JSON', async () => {
    await API.get('/api/movies/advanced-query/name-movies-roles/nm0001893')
      .expect(200)
      .expect('Content-Type', /application\/json/)
  })
